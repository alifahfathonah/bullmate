...
<script type="text/javascript">
    ...
    sUrl = '<?php echo site_url('home/note_register');?>';
        note = document.getElementById("note").value;
        lesson_id = document.getElementById("note_lesson_id").value;
        course_id = document.getElementById("note_course_id").value;
    ...
    if (note.length<=1) {
        ...
        $.ajax({
            url: sUrl,
        type : 'POST',
        data : {note : note, lesson_id : lesson_id,course_id : course_id},
        success: function(response)
            {
                result = JSON.parse(response);
            if(result.status == 1){
                    $('#setMgs').html('Note successfully done.');
                    $("#alertMegs").show();
                }
            }
        });
