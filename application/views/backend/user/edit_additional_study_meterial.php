...
<form action="<?php echo site_url('user/lessons/'.$param2.'/add'); ?>" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <select class="form-control select2" data-toggle="select2" name="lesson_type" id="lesson_type" required onchange="show_lesson_type_form(this.value)">
            ...
            <option value="meterial-xls" <?php if($lesson_details['attachment_type'] == 'xls') echo 'selected'; ?>><?php echo get_phrase('xls_file'); ?></option>
            <option value="meterial-img" <?php if($lesson_details['attachment_type'] == 'img') echo 'selected'; ?>><?php echo get_phrase('image_file'); ?></option>
        </select>
    </div>
